...
#define PAGING_MEMRAMSZ BIT(10) /* 1MB  - RAM hỗ trợ tối đa*/
#define PAGING_PAGE_ALIGNSZ(sz) (DIV_ROUND_UP(sz,PAGING_PAGESZ)*PAGING_PAGESZ)

#define PAGING_MEMSWPSZ BIT(14) /* 16MB - SWAP */
...
/* PTE BIT PRESENT */
#define PAGING_PTE_SET_PRESENT(pte) (pte=pte|PAGING_PTE_PRESENT_MASK)   //set present = 1
#define PAGING_PTE_PAGE_PRESENT(pte) (pte&PAGING_PTE_PRESENT_MASK)      // check present == 1 ?

/* USRNUM - Xác định vị trí bit lưu số người dùng hoặc thông tin user-defined */
#define PAGING_PTE_USRNUM_LOBIT 15
#define PAGING_PTE_USRNUM_HIBIT 27
/* FPN - Xác định vị trí bit (0-12) chứa Frame Page Number (FPN) (nếu trang hiện diện)*/
#define PAGING_PTE_FPN_LOBIT 0
#define PAGING_PTE_FPN_HIBIT 12
/* SWPTYP - Xác định vị trí bit (0-4) chứa loại SWAP (swap type)*/
#define PAGING_PTE_SWPTYP_LOBIT 0
#define PAGING_PTE_SWPTYP_HIBIT 4
/* SWPOFF - Xác định vị trí bit (5-25) chứa offset trong SWAP*/
...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr); //TODO
int __free(struct pcb_t *caller, int rgid); //TODO
...
/* VM prototypes -> mm-vm.c*/
int pgalloc(struct pcb_t *proc, uint32_t size, uint32_t reg_index);     // cấp phát trong bộ nhớ DATA
int pgmalloc(struct pcb_t *proc, uint32_t size, uint32_t reg_index);    // cấp phát trong bộ nhớ HEAP
